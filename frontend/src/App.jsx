...
export default function App() {
  const [tab, setTab] = useState("compose"); // compose | schedule | history
  ...
  const connect = async () => {
    ...
    try {
      ...
      if (!r.ok) { setConnectError(d.error); return; }
      setToken(tokenInput);
      setTokenInput("");
      fetchStatus();
  ...
  const schedulePost = async () => {
    ...
    try {
      const r = await fetch(`${API}/schedule`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: preview || null, topic, tone, postType, keywords, scheduledFor }),
      });
      const d = await r.json();
      if (r.ok) {
    ...
    } catch (e) { setScheduleResult({ ok: false, msg: e.message }); }
    setScheduling(false);
  };
  ...
  const s = {
    ...
    card: {
      background: "#0d1424", border: "1px solid #1a2640",
    ...
    input: {
      ...
      background: "#080c16", border: "1px solid #1a2640",
    ...
    btnGhost: {
      ...
      border: "1px solid #1a2640", background: "transparent",
    ...
    btnRed: {
      ...
      border: "1px solid #3b1f1f", background: "#1a0a0a",
  ...
  return (
    <div style={s.app}>
      ...
      <div style={s.main}>
        ...
        {!status.connected && (
          <div style={{ ...s.card, borderColor: "#1e3a5f", marginBottom: "28px" }}>
            <div style={{ marginBottom: "16px" }}>
              <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: "18px", marginBottom: "6px" }}>
                Connect LinkedIn
              </h2>
              <p style={{ fontSize: "13px", color: "#64748b", lineHeight: 1.6 }}>
                Get your access token from{" "}
                <a href="https://www.linkedin.com/developers/tools/oauth/token-generator"
            ...
            {connectError && (
              <p style={{ color: "#f87171", fontSize: "13px", marginTop: "10px" }}>{connectError}</p>
            )}
          </div>
        )}
        ...
        {/* Tabs */}
        <div style={s.tabs}>
          {["compose", "schedule", "history"].map(t => (
            <button key={t} style={s.tab(tab === t)} onClick={() => setTab(t)}>
              {t === "compose" ? "✍️ Compose" : t === "schedule" ? "📅 Scheduled" : "📋 History"}
            </button>
          ))}
        </div>
          ...
          <>
            <div style={s.card}>
              ...
              <div style={{ ...s.pills, marginBottom: "16px" }}>
                {POST_TYPES.map(t => (
                  <button key={t} style={s.pill(postType === t)} onClick={() => setPostType(t)}>{t}</button>
                ))}
              </div>
              ...
              <div style={{ ...s.pills, marginBottom: "16px" }}>
                {TONES.map(t => (
                  <button key={t} style={s.pill(tone === t)} onClick={() => setTone(t)}>{t}</button>
                ))}
              </div>
            ...
            <div style={{ display: "flex", gap: "10px", marginBottom: "16px" }}>
              <button style={{ ...s.btnPrimary, flex: 1 }} onClick={generate} disabled={generating || !topic.trim()}>
              ...
              <div style={s.card}>
                ...
                {generating ? (
                  ...
                  <>
                    ...
                    {postResult && (
                      <p style={{ color: postResult.ok ? "#4ade80" : "#f87171", fontSize: "13px", marginTop: "12px" }}>
                        {postResult.msg}
                      </p>
                    )}
                    {!status.connected && (
        ...
        {tab === "schedule" && (
          <>
            <div style={s.card}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "16px" }}>
                <div>
                  ...
                  <input type="date" style={s.input} value={scheduleDate} onChange={e => setScheduleDate(e.target.value)} />
                </div>
                <div>
                  <label style={s.label}>Time</label>
                  <input type="time" style={s.input} value={scheduleTime} onChange={e => setScheduleTime(e.target.value)} />
                </div>
              </div>
              ...
              <p style={{ fontSize: "12px", color: "#475569", marginBottom: "14px" }}>
                AI will auto-generate the post at the scheduled time unless you paste a custom post below.
              </p>

              <label style={s.label}>Custom Post Text (optional — leave blank to auto-generate)</label>
              <textarea
                style={{ ...s.textarea, marginBottom: "16px" }}
                rows={4}
              ...
              <button
                ...
                disabled={scheduling || !topic.trim() || !scheduleDate || !scheduleTime || !status.connected}
              >
                {scheduling ? "Scheduling..." : "📅 Schedule Post"}
              </button>
              {!status.connected && (
                <p style={{ color: "#f59e0b", fontSize: "12px", marginTop: "8px" }}>⚠️ Connect LinkedIn to schedule</p>
              )}
              {scheduleResult && (
                <p style={{ color: scheduleResult.ok ? "#4ade80" : "#f87171", fontSize: "13px", marginTop: "10px" }}>
                  {scheduleResult.msg}
                </p>
              )}
            </div>
            ...
            <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: "15px", marginBottom: "12px", color: "#94a3b8" }}>
              Upcoming Posts ({(status.scheduledPosts || []).filter(p => p.status === "pending").length})
            </h3>
            {(status.scheduledPosts || []).filter(p => p.status === "pending").length === 0 ? (
              <p style={{ color: "#475569", fontSize: "13px" }}>No posts scheduled yet.</p>
            ) : (
              (status.scheduledPosts || []).filter(p => p.status === "pending").map(p => (
                <div key={p.id} style={{ ...s.card, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    ...
                    {p.text && <p style={{ fontSize: "12px", color: "#475569", marginTop: "6px", fontStyle: "italic" }}>Custom text saved</p>}
                  </div>
        ...
        {tab === "history" && (
          <>
            <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: "15px", marginBottom: "16px", color: "#94a3b8" }}>
              Post History
            </h3>
